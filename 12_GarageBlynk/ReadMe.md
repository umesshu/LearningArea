...
**2026-09-17 加上操作者資訊**：想在監控頁面看到「誰」按的，在捷徑裡先加一步「取得裝置詳細資訊」→「名稱」，組合成網址參數再送出：

```
開：https://blynk.cloud/external/api/update?token={你的Token}&V0=open|{裝置名稱}
關：https://blynk.cloud/external/api/update?token={你的Token}&V0=close|{裝置名稱}
```

（`{裝置名稱}` 直接放捷徑變數，捷徑組網址時會自動 URL-encode）韌體用 `|` 分割出動作跟操作者，操作者裡如果剛好有逗號或冒號，韌體會自動換成空白，不用擔心壞掉格式；操作者這段不填也完全沒問題，等於維持舊行為。
...
**2026-09-17 修正——輪詢間隔內連續操作會漏記**：原本 V0 只回報「最新一筆」（`"open:N"`），如果兩次操作剛好落在同一個輪詢間隔內，中間那幾筆會被直接覆寫掉，樹莓派永遠看不到。改成韌體端維護「最近 8 筆」歷史，逗號分隔、新的在前塞進同一個 String（例如 `"12:close,11:open,10:close"`），樹莓派一次輪詢解析整串、把所有還沒處理過的 counter 都補回去，不會再漏記。

⚠️ 這個修法有個上限：緩衝區只留最近 **8 筆**，如果同一個輪詢間隔內操作次數超過 8 次，最舊的那幾筆還是會被擠出去、來不及被樹莓派讀到就永久遺失。輪詢間隔維持預設 **10 秒**（一度改成 2 秒試圖縮小風險，後來評估「10 秒內操作 8 次」對這個場景已經非常夠用，改回 10 秒；2 支 API/次，一天約 1.7 萬次/裝置，遠低於 Blynk 免費額度 50 萬次/裝置/天）。真的需要應付更高頻率的操作，加大韌體裡的 `OP_HISTORY_SIZE` 即可（Blynk String 欄位空間很寬裕，不是瓶頸）。
