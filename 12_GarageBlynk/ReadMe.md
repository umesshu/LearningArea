...
好處是不管樹莓派實際在哪個網路，只要能上網就能監控到；代價是**操作紀錄的時間戳記精確度受輪詢間隔影響**（預設 2 秒輪詢一次），且一樣**沒有來源裝置/IP**（跟之前一樣，觸發不經過樹莓派，技術上做不到）。

**2026-09-17 修正——輪詢間隔內連續操作會漏記**：原本 V0 只回報「最新一筆」（`"open:N"`），如果兩次操作剛好落在同一個輪詢間隔內，中間那幾筆會被直接覆寫掉，樹莓派永遠看不到。改成韌體端維護「最近 8 筆」歷史，逗號分隔、新的在前塞進同一個 String（例如 `"12:close,11:open,10:close"`），樹莓派一次輪詢解析整串、把所有還沒處理過的 counter 都補回去，不會再漏記；同時把輪詢間隔從 10 秒縮短成 2 秒，讓時間戳記更準（2 支 API/次，兩個裝置一天約 17 萬次，遠低於 Blynk 免費額度 50 萬次/裝置/天）。
